...
class io_listenToPlayer extends IO {
    ...
    think() {
        ...
        if (this.body.invuln) {
            if (this.player.command.right || this.player.command.left || this.player.command.up || this.player.command.down || this.player.command.lmb||this.player.command.alt||this.player.command.ult) {
...
class io_mapTargetToGoal extends IO {
    ...
    think(input) {
        if (input.main || input.alt||input.ult) {
            return {         
                goal: {
...
class io_nearestDifferentMaster extends IO {
    ...
    think(input) {
        ...
        if (input.main || input.alt  || input.ult || this.body.master.autoOverride) {
...
class io_minion extends IO {
    ...
    think(input) {
        ...
        if (input.target != null && (input.ult|| input.alt || input.main)) {
...
class Gun {
    constructor(body, info) {
        ...
        if (info.PROPERTIES != null && info.PROPERTIES.TYPE != null) {
            ...
            this.ultFire = (info.PROPERTIES.ULT_FIRE == null) ?
                false : info.PROPERTIES.ULT_FIRE;
    ...
    live() {
        ...
        if (this.canShoot) {
            ...
            var sprite=false;
            if(this.ultFire){sprite=this.body.control.ult;}else if(this.altFire){sprite=this.body.control.alt;}else{sprite=this.body.control.fire;}
            // Firing routines
            if (shootPermission && (this.autofire ||this.sprite)) {
                if (this.cycle >= 1) {
            ...
            function incoming(message, socket) {
                ...
                case 'C': { // command packet
                    ...
                    let target = {
                        ...
                        altcommands = m[2];
                    // Verify data
                    if (typeof target.x !== 'number' || typeof target.y !== 'number' || typeof altcommands !== 'number') { socket.kick('Weird downlink.'); return 1; }
                    if (altcommands > 255) { socket.kick('Malformed command packet.'); return 1; }
                    // Put the new target in
                    player.target = target
                    // Process the commands
                    if (player.command != null && player.body != null) {
                        player.command.ult    = (altcommands &  1)
                        player.command.sprint  = (altcommands &  2) >> 1
                        player.command.ovd = (altcommands &  4) >> 2
                        //player.altcommand.right = (altcommands &  8) >> 3
                        //player.altcommand.lmb   = (altcommands & 16) >> 4
                        //player.altcommand.mmb   = (altcommands & 32) >> 5
                        //player.altcommand.rmb  = (altcommands & 64) >> 6
                    }
                    // Update the thingy 
                    socket.timeout.set(altcommands)
                } break;
                case 't': { // player toggle
                    ...
                    switch (tog) {
                        ...
                        case 3: given = 'rvt'; break;
                        case 4: given = 'rvm'; break;
                        case 5: given = 'automove'; break;
